 ...
 * 5) 全局协调（0.1.2+）：多会话窗口内同时 429 判定为 provider 级限流并进入全局
 *    冷却（暂停所有自动 continue/迁移）；迁移互斥保证同一时刻只有一个交接，
 *    压住父代理开多个子代理时的连环迁移。
 *
...
const CONFIG_DEFAULTS = {
  ...
  providerBurstWindowMs: 30000, // provider 级限流判定窗口（毫秒）
  providerBurstSessions: 2,     // 窗口内同时 429 的会话数 ≥ 此值 → 判定 provider 级限流
  providerBurstCount: 3,        // 且窗口内 429 总数 ≥ 此值（防单会话两次就误判）
  globalCooldownMs: 60000,      // provider 限流 / 每次迁移完成后的全局冷却（毫秒）
};
...
const MIN_BURST_WINDOW_MS = 5000;
const MAX_BURST_WINDOW_MS = 300000;
const MIN_BURST_SESSIONS = 2;
const MAX_BURST_SESSIONS = 20;
const MIN_BURST_COUNT = 2;
const MAX_BURST_COUNT = 50;
const MIN_GLOBAL_COOLDOWN_MS = 5000;
const MAX_GLOBAL_COOLDOWN_MS = 600000;
...
let settings = {
  ...
  providerBurstWindowMs: CONFIG_DEFAULTS.providerBurstWindowMs,
  providerBurstSessions: CONFIG_DEFAULTS.providerBurstSessions,
  providerBurstCount: CONFIG_DEFAULTS.providerBurstCount,
  globalCooldownMs: CONFIG_DEFAULTS.globalCooldownMs,
};
...
function clampSettings() {
  ...
  const bw = Number(settings.providerBurstWindowMs);
  settings.providerBurstWindowMs = Number.isFinite(bw)
    ? Math.min(MAX_BURST_WINDOW_MS, Math.max(MIN_BURST_WINDOW_MS, Math.floor(bw)))
    : CONFIG_DEFAULTS.providerBurstWindowMs;
  const bs = Number(settings.providerBurstSessions);
  settings.providerBurstSessions = Number.isFinite(bs)
    ? Math.min(MAX_BURST_SESSIONS, Math.max(MIN_BURST_SESSIONS, Math.floor(bs)))
    : CONFIG_DEFAULTS.providerBurstSessions;
  const bc = Number(settings.providerBurstCount);
  settings.providerBurstCount = Number.isFinite(bc)
    ? Math.min(MAX_BURST_COUNT, Math.max(MIN_BURST_COUNT, Math.floor(bc)))
    : CONFIG_DEFAULTS.providerBurstCount;
  const gc = Number(settings.globalCooldownMs);
  settings.globalCooldownMs = Number.isFinite(gc)
    ? Math.min(MAX_GLOBAL_COOLDOWN_MS, Math.max(MIN_GLOBAL_COOLDOWN_MS, Math.floor(gc)))
    : CONFIG_DEFAULTS.globalCooldownMs;
...
async function apply(ctx, config = {}) {
  ...
  // 全局协调参数：可用 config 覆盖（测试 / 部署微调），不回写 settings 文件
  const eff = {
    burstWindowMs: Number.isFinite(Number(config.providerBurstWindowMs))
      ? Number(config.providerBurstWindowMs) : settings.providerBurstWindowMs,
    burstSessions: Number.isFinite(Number(config.providerBurstSessions))
      ? Math.floor(Number(config.providerBurstSessions)) : settings.providerBurstSessions,
    burstCount: Number.isFinite(Number(config.providerBurstCount))
      ? Math.floor(Number(config.providerBurstCount)) : settings.providerBurstCount,
    cooldownMs: Number.isFinite(Number(config.globalCooldownMs))
      ? Number(config.globalCooldownMs) : settings.globalCooldownMs,
  };

  // ── 全局协调状态（跨会话）：压住「多子代理同时 429 → 连环交接」───
  // 背景：父代理开 N 个子代理时，TPM 限流通常是 provider 级的 —— 所有会话
  // 同时 429。若各自独立计数迁移，会出现 N 个会话同时各自 cancel+建新会话
  // 的连环交接，且新会话立刻又 429。策略：
  //   1) 突发检测：窗口内 ≥ burstSessions 个不同会话、且 429 总数 ≥ burstCount
  //      → 判定 provider 级限流，进入全局冷却（暂停自动 continue 与自动迁移）；
  //   2) 全局互斥：同一时刻只允许一个迁移在途；
  //   3) 每次迁移成功后也进入冷却，防止其它会话紧随其后连环交接。
  const globalState = {
    inflight: false,      // 有一个迁移正在进行
    lastMigrationAt: 0,   // 上次迁移完成时间戳
    cooldownUntil: 0,     // 全局冷却截止时间戳
    burstEvents: [],      // [{ at, sid }] 最近窗口内全部会话的 429 事件
  };

  function now() { return Date.now(); }
  function pruneBurst() {
    const cutoff = now() - eff.burstWindowMs;
    while (globalState.burstEvents.length > 0 && globalState.burstEvents[0].at < cutoff) globalState.burstEvents.shift();
  }
  function inCooldown() { return now() < globalState.cooldownUntil; }
  function enterCooldown(reason) {
    const until = now() + eff.cooldownMs;
    if (until > globalState.cooldownUntil) {
      globalState.cooldownUntil = until;
      console.info(`[migrate-on-429] ⏸ 进入全局冷却 ${eff.cooldownMs}ms（${reason}），期间暂停自动 continue 与自动迁移`);
    }
  }
  // 记录一次 429 并尝试判定 provider 级限流；命中则进入冷却并返回 true。
  function recordBurst(sid) {
    if (!settings.enabled || !settings.quickOn) return false;
    pruneBurst();
    globalState.burstEvents.push({ at: now(), sid });
    const distinct = new Set(globalState.burstEvents.map((e) => e.sid));
    if (distinct.size >= eff.burstSessions && globalState.burstEvents.length >= eff.burstCount) {
      enterCooldown(`检测到 provider 级限流：窗口内 ${globalState.burstEvents.length} 次 429 来自 ${distinct.size} 个会话`);
      globalState.burstEvents = [];
      return true;
    }
    return false;
  }
  ...
  // ── 交接内容精简（dsh-argp 思路：去重、剪孤立、保留引用链；零 LLM、确定性）────
  // dsh 宿主会把 workspace/项目指令与技能目录以 system-reminder 用户消息注入；
  // 新会话里宿主同样会重新注入它们，交接文档再抄一份纯属重复 token —— 剔除。
  function isSystemReminderText(text) {
    if (!text) return true;
    return (
      text.startsWith("<system-reminder>") ||
      text.includes("Instructions from:") ||
      text.includes("Additional instructions from:") ||
      text.includes("These instructions apply to work under") ||
      text.includes("The following skills are available in this session") ||
      text.includes("A skill is a reusable set of task-specific instructions")
    );
  }

  // 迁移链去嵌套：若首条用户消息是上一代迁移种子（"[系统交接] … —— 交接总结 ——"），
  // 递归解开每一层外壳，取最深一层「## 原始任务」的真实任务文本；中间各代交接
  // 整段丢弃（完整历史仍逐代留在 ~/.dsh/migrations 文档里，交接规则 3 给路径按需读取）。
  function unwrapHandoverSeed(text) {
    const marker = "—— 交接总结 ——";
    let cur = String(text || "").trim();
    for (let depth = 0; depth < 8; depth++) {
      if (!cur.startsWith("[系统交接]")) break;
      const sep = cur.indexOf(marker);
      if (sep === -1) break;
      cur = cur.slice(sep + marker.length);
      const m = cur.match(/\n##\s*原始任务\s*\n([\s\S]+?)(?=\n##\s*(?:用户后续指示|已完成进展|涉及文件|待办\/未完成|LLM 精炼总结)(?:（[^）]*）)?\s*\n|$)/);
      if (!m) break;
      let inner = m[1].trim();
      inner = inner.split(/\n\s*---\s*\n/)[0].trim(); // 磁盘文档里 LLM 精炼段用 --- 分隔，一并切掉
      if (inner.length < 2) break;
      cur = inner;
    }
    return cur.length > 200000 ? cur.slice(0, 200000) + "…" : cur;
  }
  ...
  function buildHandoverMarkdown(agent, opts) {
    ...
    // 原始任务取首条「真实用户消息」：先滤 system-reminder，再解迁移链嵌套。
    const realUserMsgs = userMsgs.filter((u) => !isSystemReminderText(u.text));
    const unwrapped = realUserMsgs.length ? unwrapHandoverSeed(realUserMsgs[0].text) : "";
    const originalTask = unwrapped || (userMsgs.length ? userMsgs[0].text : "(未找到原始任务，请查看交接文档)");
    // 后续指示：剔除 system 提醒、与原始任务重复、与已收录内容重复的消息，逐条限长
    const followups = [];
    {
      const seen = new Set();
      for (const u of userMsgs.slice(1)) {
        if (isSystemReminderText(u.text)) continue;
        const t = u.text.trim();
        if (!t || t === originalTask) continue;
        const key = t.slice(0, 120);
        if (seen.has(key)) continue;
        seen.add(key);
        followups.push(clipText(t, 600));
      }
    }
    // 已完成进展：取最近 8 条去重后的非空助手输出（逐条限长渲染）
    const recent = [];
    {
      const seen = new Set();
      for (let i = assistantTexts.length - 1; i >= 0 && recent.length < 8; i--) {
  ...
  async function runMigration(agent, s, reason) {
    ...
    try {
      ...
      globalState.inflight = false;
      globalState.lastMigrationAt = now();
      enterCooldown("上次迁移完成，冷却防连环交接");
  ...
  function startMigration(agent, s, reason) {
    ...
    if (globalState.inflight) {
      console.info(`[migrate-on-429] 会话 ${agent.id} 触发迁移被跳过：已有迁移在途（${reason}）`);
      return;
    }
    if (inCooldown()) {
      console.info(`[migrate-on-429] 会话 ${agent.id} 触发迁移被跳过：全局冷却中，剩 ${Math.ceil((globalState.cooldownUntil - now()) / 1000)}s（${reason}）`);
      return;
    }
    globalState.inflight = true;
          ...
          handler: async (req, res) => {
            ...
            res.end(JSON.stringify({
              ...
              globalCooldown: {
                ...
                inflight: globalState.inflight,
                lastMigrationAt: globalState.lastMigrationAt,
              },
            }));
          },
        },
        ...
        },
        {
          ...
          handler: async (req, res) => {
            ...
            try {
              ...
              if (body.providerBurstWindowMs !== undefined) {
                const bw = Number(body.providerBurstWindowMs);
                if (!Number.isFinite(bw) || bw < MIN_BURST_WINDOW_MS || bw > MAX_BURST_WINDOW_MS) errors.push(`providerBurstWindowMs 必须是 ${MIN_BURST_WINDOW_MS}-${MAX_BURST_WINDOW_MS} 的整数`);
                else settings.providerBurstWindowMs = Math.floor(bw);
              }
              if (body.providerBurstSessions !== undefined) {
                const bs = Number(body.providerBurstSessions);
                if (!Number.isFinite(bs) || bs < MIN_BURST_SESSIONS || bs > MAX_BURST_SESSIONS) errors.push(`providerBurstSessions 必须是 ${MIN_BURST_SESSIONS}-${MAX_BURST_SESSIONS} 的整数`);
                else settings.providerBurstSessions = Math.floor(bs);
              }
              if (body.providerBurstCount !== undefined) {
                const bc = Number(body.providerBurstCount);
                if (!Number.isFinite(bc) || bc < MIN_BURST_COUNT || bc > MAX_BURST_COUNT) errors.push(`providerBurstCount 必须是 ${MIN_BURST_COUNT}-${MAX_BURST_COUNT} 的整数`);
                else settings.providerBurstCount = Math.floor(bc);
              }
              if (body.globalCooldownMs !== undefined) {
                const gc = Number(body.globalCooldownMs);
                if (!Number.isFinite(gc) || gc < MIN_GLOBAL_COOLDOWN_MS || gc > MAX_GLOBAL_COOLDOWN_MS) errors.push(`globalCooldownMs 必须是 ${MIN_GLOBAL_COOLDOWN_MS}-${MAX_GLOBAL_COOLDOWN_MS} 的整数`);
                else settings.globalCooldownMs = Math.floor(gc);
              }
              if (errors.length > 0) {
              ...
              saveSettingsNow(b);
              res.writeHead(200, { "content-type": "application/json" });
              res.end(JSON.stringify({ ok: true, migrateThreshold: settings.migrateThreshold, windowMs: settings.windowMs, llmSummary: settings.llmSummary, providerBurstWindowMs: settings.providerBurstWindowMs, providerBurstSessions: settings.providerBurstSessions, providerBurstCount: settings.providerBurstCount, globalCooldownMs: settings.globalCooldownMs }));
            } catch {
              res.writeHead(400, { "content-type": "application/json" });
        ...
        },
        {
          ...
          handler: async (req, res) => {
            ...
            try {
              ...
              if (globalState.inflight) {
                res.writeHead(409, { "content-type": "application/json" });
                res.end(JSON.stringify({ ok: false, error: "已有会话正在迁移中，请稍候" }));
                return;
              }
              if (!settings.enabled) {
                res.writeHead(400, { "content-type": "application/json" });
  ...
  const offRequestError = ctx.on("agent/request-error", async (payload, next) => {
    try {
      ...
      prune(s, now);
      // provider 级突发检测：多个会话在窗口内同时 429 → 判定全局限流并进入冷却，
      // 本轮不再按会话触发迁移（startMigration 也有冷却闸兜底）。
      if (recordBurst(sid)) return next();
  ...
  const offSessionEvent = ctx.on("session/event", (session, event) => {
    ...
    if (inCooldown()) return; // 全局冷却中：不发 continue、不迁移，等 provider 限流缓解
