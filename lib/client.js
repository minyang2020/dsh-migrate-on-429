...
window.__ModuleLoader__.load({
  id: "@minyang2026/dsh-migrate-on-429",
  factory: function (require) {
    ...
    function updateBadge() {
      ...
      if (state.cooldown && state.cooldown.active) {
        ...
        badgeEl.textContent = "⏸";
        badgeEl.title = "provider 级限流冷却中（多个会话同时 429），已暂停自动迁移防连环交接，剩余约 " + Math.ceil((state.cooldown.until - Date.now()) / 1000) + "s";
        return;
      }
    ...
    function pollStatus() {
      fetch("/api/migrate-on-429/state")
        ...
        .then(function (data) {
          ...
          state.cooldown = data.globalCooldown || null;
